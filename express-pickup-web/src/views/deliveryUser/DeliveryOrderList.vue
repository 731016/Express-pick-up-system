<template>
    <div>
            ...
            <div>
                <el-descriptions :column="1" direction="horizontal" border>
                    ...
                    <el-descriptions-item v-if="currentdialog.paymentInfo">
                        ...
                        {{currentdialog.paymentInfo.paymentStatus_cnName}}
                    </el-descriptions-item>
                    <el-descriptions-item>
                        <template slot="label">
                            <i class="el-icon-tickets "></i>
                        ...
                        {{currentdialog.orderStatus_cnName}}
                    </el-descriptions-item>
                    <el-descriptions-item>
                        <template slot="label">
                            <i class="el-icon-tickets"></i>
        ...
        <el-dialog title="订单评价" :visible.sync="dialogRateVisible"
            ...
            <div slot="footer" class="dialog-footer">
                <el-button @click="evaluate(currentdialog.orderNumber)">评 价</el-button>
            </div>
        </el-dialog>
    </div>
    ...
    export default {
        ...
        methods: {
            ...
            getFilterData() {
                ...
                console.log('发送ajax' + userId);
                //todo 【根据查询条件】查询获取当前用户，没有被删除或撤销的订单
                // axios.post('xxx', {
            ...
            rowChange() {
                ...
                selectedAllData.forEach(function (item) {
                    if (item.orderStatus == 10) {
                        revokeIds.push(item.orderNumber);
                    } else if (item.orderStatus == 40 || item.orderStatus == 30) {
                        deleteIds.push(item.orderNumber);
                    }
                });
